    ...
    try:
        ...
        with engine.connect() as conn:
            n_c = conn.execute(text(
                "SELECT COUNT(*) FROM candidates WHERE company_id=:cid"
            ), {"cid": company_id}).scalar()
            n_jm = conn.execute(text(
                "SELECT COUNT(*) FROM job_matches WHERE company_id=:cid"
            ), {"cid": company_id}).scalar()
            n_oc = conn.execute(text(
                "SELECT COUNT(*) FROM outreach_campaigns WHERE company_id=:cid"
            ), {"cid": company_id}).scalar()
        return {
            "company_id": company_id,
            "candidates": n_c,
