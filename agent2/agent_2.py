...
def _score_role_fit(profile: str, job_desc: str, required_skills: List[str]) -> dict:
    ...
    jd_keywords = set(re.findall(r'\b[a-z]{4,}\b', jd)) - {"with","that","this","have","from","will","your","they","their","been","also","more"}
...
def _score_culture_fit(profile: str) -> dict:
    ...
    # Startup experience
    startup_signals = ["startup","early-stage","seed","series a","series b","founding","pre-launch","0 to 1","greenfield"]
    startup_pts = 6.0 if any(s in text for s in startup_signals) else 2.0

    # Cross-functional collaboration
    collab_signals = ["cross-functional","collaborated","team of","led team","worked with","partnership","stakeholder"]
    collab_pts = min(sum(1 for s in collab_signals if s in text) * 1.5, 5.0)

    # Rapid prototyping / shipping
    ship_signals = ["shipped","launched","prototype","mvp","rapid","hackathon","built in","24 hour","48 hour","weekend"]
